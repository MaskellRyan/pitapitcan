     ...
     *
  	...
  	Map: function(aMapCanvas) {
		 ...
		 *			the specified center or homepoint
  	  	 * returns: none
  	  	 */
        this.__buildMap = function(center, aCallback) {
            if (this.gmap === null)
            {
                this.options = {
		...
		this.putMarkers = function(markerList, animation) {
            ...
            for (var markerNumber = 0 ; markerNumber < markerCount; ++markerNumber) {
				...
				if (this.loadedOnce) {
                    ...
                    global_markers[markerList[markerNumber].id] = markerList[markerNumber];
         ...
         * @return {undefined}
         */
  	  	this.doGeocode = function(aCallback) {
			var geocoder = new google.maps.Geocoder();
  	  	  	var _this = this;

  	  	  	geocoder.geocode(
				{
					'address': this.address + ", usa" //TODO: made changes here for 'only usa'
  	  	  	  	},
        ...
        */
		...
		this.loadMarkers = function(center, radius, tags) {
            ...
            ajax.send(action, function (response) {
                    ...
                    jQuery(".search_result_btn_info").click(moreInfoButtonClick);

                    jQuery(".search_result_order").each(function(){
		...
		this.createSidebar = function(aMarker) {
			...
			var zip = aMarker.zip;
            var storefrontImage = aMarker.storefront_image;
            var menu_pdf = aMarker.menu_pdf;
            var orderOnlineUrl = aMarker.order_online_url;

            var breakfast = aMarker.breakfast;
            var order_online = aMarker.order_online;
            var after_midnight = aMarker.after_midnight;
            var delivery = aMarker.delivery;
         ...
         var decodedHours = jQuery("<div/>").html(aMarker.hours).text();
 		 ...
 		 div.innerHTML = slplus.results_string.format(
                        ...
                        aMarker.country,
                        decodedHours,
             (storefrontImage == "" ? "mapPlaceholderImage.jpg" :  storefrontImage),
                        orderOnlineUrl == null ? '' : orderOnlineUrl,
                        breakfast,
                        order_online,
                        after_midnight,
                        delivery,

                            menu_pdf, //(menu_pdf == "" ? "Default_Menu.pdf" : menu_pdf),
			...
			div.className = 'results_entry '
                + (breakfast == 0 ? 'noBreakfast' : '') + ' '
                + (order_online == 0 ? 'noOrderonline' : '') + ' '
                + (after_midnight == 0 ? 'noAfterMidnight' : '') + ' '
		+ (menu_pdf == "" || menu_pdf == "none" ? 'noMenu' : '') + ' '
                + (delivery == 0 ? 'noDelivery' : '');
 ...
 * @returns {undefined}
 */
function InitializeTheMap(aSuccessCallback) {
        ...
        if (sensor.LocationSupport) {
            sensor.currentLocation(
                ...
                function(loc) {
                    ...
                    cslmap.__buildMap(new google.maps.LatLng(loc.coords.latitude, loc.coords.longitude), aSuccessCallback);
                },
                ...
                function(error) {
                    clearTimeout(sensor.location_timeout);
                	cslmap = new csl.Map();
                    cslmap.doGeocode(aSuccessCallback);
                }
            );
...
/*
        //Setup image map click callback for states
        jQuery("#state_map_map area").click(function(){

            var stateAbbreviation = jQuery(this).data('state').toLowerCase();


            var stateName;

            for(var i = 0; i < stateAbbreviations.length; i++){
                if(stateAbbreviations[i][0] == stateAbbreviation){
                    stateName= stateAbbreviations[i][1];
                    break;
                }
            }


            jQuery("#state_map_input_address").val(stateName.toUpperCase());
        });

*/
        //Setup FIND! button callback
        jQuery("#state_map_input_button").click(function(){
            DoMapSearch();
        });
/*
        jQuery('#state_map_input_address').on( 'keydown', function( e ) {
...
function DoMapSearch(){
	...
	jQuery("#sl_div").show();
        InitializeTheMap(function(){jQuery("#searchForm").submit();});
...
function moreInfoButtonClick(){
    var markerNumber = jQuery(this).data("marker_number");

    ShowStoreInfo(markerNumber);

}


function ShowStoreInfo(markerNumber){


    jQuery(".searchResultDiv").removeClass("active");
    jQuery(".searchResultDiv").addClass("inactive");

    jQuery("#searchResultDiv_" + markerNumber).removeClass("inactive").addClass("active");


    //Show storefront image
    ShowStoreFrontImage(markerNumber);

}

var showingStoreFrontImage = false;

function ShowStoreFrontImage(markerNumber){

    var image_name = global_markers[markerNumber]["storefront_image"];

    if(image_name != ""){
        jQuery("#storefont_image_img").attr("src", "wp-content/plugins/store-locator-le/images/" + image_name );

        if(!showingStoreFrontImage){
            var $map = jQuery("#map");
            var $img = jQuery("#storefont_image_cell");

            jQuery("#storefont_image_cell").css("max-height", "334px");


            //Tween height of map and image
    ...
    showingStoreFrontImage = true;
}



function HideStoreFrontImage(){
